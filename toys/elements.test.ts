...
function mount(tag: 'c0ffee-mirror' | 'c0ffee-swatch', hex?: string): HTMLElement & ColorInterface {
...
test('<c0ffee-mirror> seeds value + hex from the hex attribute', () => {
  const el = mount('c0ffee-mirror', '3A7BD5');
...
test('<c0ffee-mirror> with no hex defaults to the namesake #C0FFEE', () => {
  const el = mount('c0ffee-mirror');
  expect(el.hex).toBe('C0FFEE');
});

test('<c0ffee-mirror> reseeds when its hex attribute changes after mount', () => {
  ...
  // pre-connect seed.
  const el = mount('c0ffee-mirror', '000000');
  el.setAttribute('hex', 'FF6600');
  expect(el.value).toEqual({ r: 255, g: 102, b: 0 });
  expect(el.hex).toBe('FF6600');
});

test('<c0ffee-mirror> RGB slider edit updates the value and emits a composed colorchange', () => {

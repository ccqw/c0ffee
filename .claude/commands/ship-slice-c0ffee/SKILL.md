...
- Use the **`ollama-peer-review`** skill (`/peer-review` is **not installed** here). Host `100.78.49.57:11434`, model **`qwen3-coder-next:latest`** (code-specialised; `gemma3:27b` is NOT on the box).
- **Triage hard** — it hallucinates (invented a CRIT on C0FFEE-19 that the passing build refuted). Cross-check every finding against the diff before acting.

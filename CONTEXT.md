...
- "**Venn palette**" → "**Additive Venn**" (renamed in the 2026-06-02 console-redesign grill). The original name carried a painter's-palette→light subversion pun; it was dropped for a term that states the phenomenon directly (additive light mixing). The glossary is updated; the code (`.venn*` classes/comments), README, and ROADMAP still say "Venn palette" — a rename sweep is pending (same shape as the C0FFEE-20 mirror→console pass).

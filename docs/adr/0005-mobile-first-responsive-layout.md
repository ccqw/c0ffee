...
The site is designed **mobile-first**: every layout is authored for narrow screens first, and the wide-screen (desktop) layout is the *widened variant* that falls out — not the primary design. This **supersedes ADR-0004's commitment to a left-pinned Companion mirror**. Everything else in ADR-0004 stands; only the word "left" is replaced.
...
- **The site root (`/`) is the console's Playground.** Home *is* the flagship console, full-bleed — so its mobile experience is the product's first impression, which is the strongest reason mobile leads.
...
- **Home = the console Playground**, so the console's narrow-screen experience is the front door — designing it first is designing the thing users hit first.
...
- ADR-0004 still uses the pre-rename word "mirror"; the `mirror → console` rename pass (tracked separately) updates that wording.
